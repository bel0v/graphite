...
describe('groups', () => {
  it('should add groups to node when nodesGroups updates', () => {
    ...
    d.nodesGroups.addMembers([1], 'test group')
    expect(d.nodes).toEqual([{ id: 1, groups: ['test group'], group: 'test group' }, { id: 2 }])
    d.nodesGroups.addMembers([1, 2], 'test group 2')
    expect(d.nodes).toEqual([
      { id: 1, groups: ['test group', 'test group 2'], group: 'test group 2' },
      { id: 2, groups: ['test group 2'], group: 'test group 2' }
    ])
  })
  it('should add groups to edge when edgesGroups updates', () => {
    const d = data({ edges: [{ from: 1, to: 2 }, { from: 2, to: 3 }] })
    const edges = d.edges
    d.edgesGroups.addMembers([edges[0].id], 'test group')
    expect(d.edges[0]).toMatchObject(
      { from: 1, to: 2, groups: ['test group'] }
    )
    d.edgesGroups.addMembers([edges[0].id, edges[1].id], 'test group 2')
    expect(d.edges[0]).toMatchObject(
